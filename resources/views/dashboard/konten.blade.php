    ...
    <div class="max-w-3xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form action="{{ route('settings.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <div
                    class="group relative overflow-hidden rounded-2xl bg-slate-100 shadow-sm h-64 w-48 mt-2 border border-slate-200">
                    @if(!empty($settings['foto_pria']))
                        <img src="{{ asset($settings['foto_pria']) }}" id="preview_foto_pria"
                            class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_pria"
                            class="hidden flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>
                            <span class="text-xs font-medium">Pilih Foto</span>
                        </div>
                    @else
                        <img src="" id="preview_foto_pria"
                            class="hidden h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_pria"
                            class="flex flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>
                            <span class="text-xs font-medium">Pilih Foto</span>
                        </div>
                    @endif
                    <div
                        ...
                        <label for="foto_pria_input"
                            class="cursor-pointer h-10 w-10 rounded-full bg-white text-slate-700 hover:text-blue-500 transition-colors shadow-sm flex items-center justify-center"
                            title="Ubah Foto">
                            <i class="fa-solid fa-pen"></i>
                        </label>
                        <input type="file" id="foto_pria_input" name="foto_pria" accept="image/*" class="hidden"
                            onchange="previewImage(this, 'preview_foto_pria', 'placeholder_foto_pria')" />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div
                    ...
                    @if(!empty($settings['foto_wanita']))
                        <img src="{{ asset($settings['foto_wanita']) }}" id="preview_foto_wanita"
                            class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_wanita"
                            class="hidden flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>
                            <span class="text-xs font-medium">Pilih Foto</span>
                        </div>
                    @else
                        <img src="" id="preview_foto_wanita"
                            class="hidden h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_wanita"
                            class="flex flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>
                    ...
                    <div
                        class="absolute inset-0 bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-center">
                        <label for="foto_wanita_input"
                            class="cursor-pointer h-10 w-10 rounded-full bg-white text-slate-700 hover:text-blue-500 transition-colors shadow-sm flex items-center justify-center"
                            title="Ubah Foto">
                            <i class="fa-solid fa-pen"></i>
                        </label>
                        <input type="file" id="foto_wanita_input" name="foto_wanita" accept="image/*" class="hidden"
                            onchange="previewImage(this, 'preview_foto_wanita', 'placeholder_foto_wanita')" />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div
                    class="group relative overflow-hidden rounded-2xl bg-slate-100 shadow-sm h-64 w-full mt-2 border border-slate-200">
                    @if(!empty($settings['foto_cover']))
                        <img src="{{ asset($settings['foto_cover']) }}" id="preview_foto_cover"
                            class="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_cover"
                            class="hidden flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>
                            <span class="text-xs font-medium">Pilih Foto</span>
                        </div>
                    @else
                        <img src="" id="preview_foto_cover"
                            class="hidden h-full w-full object-cover transition-transform duration-300 group-hover:scale-105">
                        <div id="placeholder_foto_cover"
                            class="flex flex-col items-center justify-center h-full w-full text-slate-400 bg-slate-50 gap-2">
                            <i class="fa-regular fa-image text-3xl"></i>

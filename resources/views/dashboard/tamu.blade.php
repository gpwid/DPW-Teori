...
<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm h-full flex flex-col">
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <button class="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50">
                Export CSV
            </button>
                    ...
                    <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                        <td class="py-4 pl-2">
                            <div class="flex items-center gap-3">
                                <div class="flex h-8 w-8 items-center justify-center rounded-full {{ $colorClass }} text-xs font-bold shadow-sm">
                                    {{ $initials }}
                                </div>
                        ...
                        <td class="py-4 text-center text-slate-400 text-xs truncate max-w-[120px]">
                            @if (!empty($guest->link_undangan))
                                <a href="{{ $guest->link_undangan }}" target="_blank" class="hover:text-primary transition-colors">
                        ...
                        <td class="py-4 pr-2 text-right">
                            <div class="flex items-center justify-end gap-3">
                                <button type="button"
                                    ...
                                    data-status="{{ $guest->status_hadir }}"
                                    data-plusone="{{ $guest->plusone ?? 0 }}"
...
<div id="addModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm px-4 transition-all">
...
<div id="editModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm px-4 transition-all">
    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <div class="mb-4 flex items-center justify-between">
